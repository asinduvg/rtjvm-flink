...
object CustomSources {
  ...
  def demoSourceFunction(): Unit = {
    val numberStream: DataStream[Long] = env.addSource(new RandomNumberGeneratorSource(10))
    numberStream.print()
    env.execute()
  }
  ...
  def demoRichSourceFunction(): Unit = {
    val numberStream: DataStream[Long] = env.addSource(new RichRandomNumberGeneratorSource(10))
    numberStream.print()
    env.execute()
  }
  ...
  class RichParallelRandomNumberGeneratorSource(minEventsPerSecond: Double)
    ...
    // called ONCE, when the function is instantiated
    // SourceFunction/RichFunction runs on a (single) dedicated thread
  ...
  def demoRichParallelSourceFunction(): Unit = {
    val numberStream: DataStream[Long] = env.addSource(new RichParallelRandomNumberGeneratorSource(10)).setParallelism(6)
    numberStream.print()
    env.execute()
  }
